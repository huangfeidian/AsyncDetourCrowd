...
public:
	...
	/// Adds a new agent to the crowd.
	///  @param[in]		idx		The request agent idx
	///  @param[in]		pos		The requested position of the agent. [(x, y, z)]
	///  @param[in]		params	The configutation of the agent.
	/// @return if we can add agent with idx return true else return false
	bool addAgent(const int idx, const dtReal_t* pos, const ExtendDetourCrowdAgentParams* params);
